...
public class GestorUsuarios {
	...
	public void crearUsuario(String login, String clave, String nombre, String tipoUsuario) 
		...
		if (mapaUsuarios.containsKey(login)) // existe el login?
			...
			case "Cliente":
				...
				break;
			case "Empleado":
				u = new Empleado(login, clave, nombre);
				break;
			case "Administrador":
	...
	public void empleadoMenosAnuncio(List<ControladorSesionEmpleado> cs) {
		String menosAnuncios = "";
		int anuncios = 20;
		for(int i = 0; i < this.mapaUsuarios.size(); i++) {
			if(this.mapaUsuarios.get("emp" + i).getNombre().equals(cs.get(i).getEmple().getNombre())) {
				if(GestorAnuncio.listarAnunciosPendientes().size() < anuncios) {
					//anuncios = GestorAnuncio.get(i).getPend().size();
					menosAnuncios = cs.get(i).getEmple().getNombre();
				}
			}
		}
		
		System.out.println("El empleado " +  menosAnuncios + " tiene menos carga de trabajo.");
	}
}